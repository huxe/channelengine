...
class inheritProductCategory(models.Model):
    ...
    @api.onchange('name','display_name','category_code','ean_number')
    def _set_sync_false(self):
        for record in self:
            record.category_sync = False
...
class inheritProductTemplate(models.Model):
    ...
    @api.onchange('name','product_template_default_code','categ_id','ean_number','qty_available','list_price','standard_price','product_images')
    def _set_sync_false(self):
        for record in self:
            record.product_template_sync = False
...
class inh_Product(models.Model):
    ...
    @api.onchange('name','display_name','default_code','ean_number','product_tmpl_id','list_price','standard_price','product_images','product_template_variant_value_ids','categ_id')
    def _set_sync_false(self):
        for record in self:
            record.product_sync = False
